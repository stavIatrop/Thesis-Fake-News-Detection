...
vectorizer = TfidfVectorizer(sublinear_tf = True, max_df = 0.56, stop_words=stopwords)
...
train_scores, valid_scores = validation_curve(SVC(random_state=42), X_train, y_train, "gamma", x, cv=5, verbose=1000, n_jobs=-1, scoring='accuracy')
...
train_scores, valid_scores = validation_curve(SVC(gamma='auto', random_state=42), X_train, y_train, "C", x , cv=5, verbose=1000, n_jobs=-1, scoring='accuracy')
...
x = [1, 2]
train_scores, valid_scores = validation_curve(SVC(gamma=1, random_state=42), X_train, y_train, "kernel", ['linear', 'rbf'] , cv=5, verbose=1000, n_jobs=-1, scoring='f1')
