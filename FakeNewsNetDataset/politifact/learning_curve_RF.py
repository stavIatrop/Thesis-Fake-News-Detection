...
# x = range(1, 20)
# train_scores, valid_scores = validation_curve(RandomForestClassifier(), X_train, y_train, "max_depth", x ,   cv=5, verbose=1000, n_jobs=-1, scoring='f1')
...
x =range(2, 502)
train_scores, valid_scores = validation_curve(RandomForestClassifier(), X_train, y_train, "min_samples_split", x,   cv=5, verbose=1000, n_jobs=-1)
...
x =range(2, 502)
train_scores, valid_scores = validation_curve(RandomForestClassifier(), X_train, y_train, "n_estimators", x,   cv=5,  verbose=1000, n_jobs=-1)
