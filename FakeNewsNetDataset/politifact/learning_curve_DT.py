...
# x =range(1, 20)
# train_scores, valid_scores = validation_curve(DecisionTreeClassifier(), X_train, y_train, "max_depth", x,   cv=5, verbose=1000, n_jobs=-1, scoring='f1' )
...
# train_scores, valid_scores = validation_curve(DecisionTreeClassifier(), X_train, y_train, "min_samples_split", x,   cv=5, verbose=1000, n_jobs=-1, scoring='f1')
# print("Scores ready.")
# train_scores = np.mean(train_scores, axis=1)
# valid_scores = np.mean(valid_scores, axis=1)
...
x = [1, 2]
print("Calculating scores...")
train_scores, valid_scores = validation_curve(DecisionTreeClassifier(), X_train, y_train, "criterion", ['entropy', 'gini'],   cv=5, verbose=1000, n_jobs=-1)
print("Scores ready.")
train_scores = np.mean(train_scores, axis=1)
valid_scores = np.mean(valid_scores, axis=1)
...
plt.ylabel("Score")
plt.axis([0, max(x) + 1, min(min(train_scores), min(valid_scores)), max(max(train_scores), max(valid_scores)) + 0.2])
