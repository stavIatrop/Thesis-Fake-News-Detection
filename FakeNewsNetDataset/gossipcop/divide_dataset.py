...
X = pd.read_csv("balanced_final.csv", ",", usecols=['id', 'text', 'label'])
...
df_train.to_csv('train_gossipcop_balanced.csv',sep=',',index = False ,header = False)
print("train made.")

df_test = pd.DataFrame(list_test)
df_test.to_csv('test_gossipcop_balanced.csv',sep=',',index = False ,header = False)
print("test made.")
