...
# df = pandas.read_csv("stemmed_gossipcop.csv" , ",")
...
# list_fake.to_csv('fake_gossipcop.csv',sep=',',index = False ,header = False)

# list_true = pandas.DataFrame(list_true)
# list_true.to_csv('true_gossipcop.csv',sep=',',index = False ,header = False)\\


# filename = "true_gossipcop.csv"
# n = 13918
# s = 4126 #desired sample size
# skip = sorted(random.sample(range(1,n+1),n-s)) #the 0-indexed header will not be included in the skip list
# df = pandas.read_csv(filename, skiprows=skip)

# df.to_csv('true_sampled.csv', header=['id', 'text', 'label'])

df = pandas.read_csv('balanced.csv', header=None)

ds = df.sample(frac=1)

ds.to_csv('balanced_final.csv')