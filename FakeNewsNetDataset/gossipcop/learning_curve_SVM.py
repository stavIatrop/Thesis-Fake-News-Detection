...
vectorizer = TfidfVectorizer(sublinear_tf = True, max_df = 0.29, stop_words=stopwords)
...
x = [1, 2]
train_scores, valid_scores = validation_curve(SVC(gamma=1, random_state=42), X_train, y_train, "kernel", ['linear', 'rbf'] ,   cv=5, verbose=1000, n_jobs=-1, scoring='accuracy')
